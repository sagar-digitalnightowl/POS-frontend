...
const AddAlertandModification = () => {
  ...
  const addAlertAndModification = async (data) => {
    setLoading(true);
    try {
      const res = await apiCall({
        method: "post",
        url: `/admin/nhra/alertModification/addAlertModification`,
        data: data
      })
      if (res.status === 201) {
      ...
      setError(error.message);
      console.log("Error add alert and modification : ", error)
    } finally {
      setLoading(false);
    }
  }
...
export default AddAlertandModification
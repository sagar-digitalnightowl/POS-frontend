...
const AllRecall = () => {
    ...
    const getAllRecalls = async () => {
        ...
        try {
            ...
            const res = await apiCall({
                method: "get",
                url: `/admin/nhra/recall/getAllRecall?${param.toString()}`
            })
            if (res.status === 200) {
                setData(res?.data?.result)
                setTotalPage(res?.data?.res)
            } else {
                setError(res?.data?.message)
            }
        } catch (error) {
            setError(error.message);
            console.log("Error add new recall : ", error)
        } finally {
            setLoading(false);
        }
    }
    ...
    const deleteRecall = async (id) => {
        ...
        try {
            const res = await apiCall({
                method: "delete",
                url: `/admin/nhra/recall/deleteRecall/${id}`
            })
            if (res.status === 200) {
                getAllRecalls();
            } else {
            ...
            setError(error.message);
            console.log("Error delete recall : ", error)
        } finally {
            setLoading(false);
        }
    }
                    ...
                    <section className="content">
                        <div className="box box-primary">
                            <div className="box-header">
                                <h3 className="box-title"> All Recalls </h3>
                                <div className="box-tools">
                                    <Link to="/recalls/create" className="btn btn-block btn-primary">
                                        ...
                                        <tbody>
                                            {data.length > 0 ? (data.map((group, index) => (
                                                <tr key={index}>
                                                    ...
                                                    <td>{group?.batchNo?.batchNo || "-"}</td>
                                                    <td>
                                                        <Link to="/ar/1" className="btn bg-green btn-sm btn-flat" title="View" style={{ marginBottom: 3 }}>
                                                            <FontAwesomeIcon icon={faEye} />
                                                        </Link>
                                                        &nbsp;
                                                        <Link to={`/recalls/edit/${group?._id}`} className="btn bg-purple btn-sm btn-flat" title="Edit" style={{ marginBottom: 3 }}>

...
const AuthorizedRepresentative = () => {
    ...
    const getAllAuthorizedRepresentatives = async () => {
        setLoading(true);
        try {
            ...
            const res = await apiCall({
                method: "get",
                url: `/admin/nhra/authorizedRepresentativeList/getAllAuthorizedRepresentative?${param.toString()}`,
            })
    ...
    const deleteAuthorizedRepresentative = async (id) => {
        setLoading(true);
        try {
            const res = await apiCall({
                method: "delete",
                url: `/admin/nhra/authorizedRepresentativeList/deleteAuthorizedRepresentative/${id}`,
            })

            if (res.status === 200) {
                getAllAuthorizedRepresentatives();
            } else {
                setError(res?.data?.message)
            }
            ...
            setError(error.message);
            console.log("Error in delete Authorized Representative : ", error)
        } finally {
            setLoading(false);
        }
    }
                    ...
                    <section className="content">
                        <div className="box box-primary">
                            <div className="box-header">
                                <h3 className="box-title">All Authorized Representative</h3>
                                <div className="box-tools">
                                    <Link to="/add-authorized-representative" className="btn btn-block btn-primary">
                                        ...
                                        <tbody>
                                            {data.length > 0 ? (data.map((group, index) => (
                                                <tr key={index}>
                                                    ...
                                                    <td>
                                                        <Link className="btn bg-green btn-sm btn-flat" title="View" style={{ marginBottom: 3 }}>
                                                            <FontAwesomeIcon icon={faEye} />
                                                        </Link>
                                                        &nbsp;
                                                        <Link to={`/edit-authorized-representative/${group?._id}`} className="btn bg-purple btn-sm btn-flat" title="Edit" style={{ marginBottom: 3 }}>
                                                            <FontAwesomeIcon icon={faEdit} />
                                                        </Link>
                                                        &nbsp;
                                                        <button onClick={() => deleteAuthorizedRepresentative(group?._id)} className="btn btn-warning btn-sm btn-flat deletebtn" id="delete_product0" data-id={1} attr_delete="https://medipro.affinity-me.com/ar/1" type="submit" title="Delete">

...
const EditAdverseEvent = () => {
  ...
  const fetchAdverseEventById = async () => {
    setLoading(true);
    try {
      const res = await apiCall({
        method: "get",
        url: `/admin/nhra/adverseEventList/getAdverseEvent/${id}`
      })
      if (res.status === 200) {
        setFormData({ ...res?.data?.result, supportiveDocuments: null, signature: null })
      } else {
      ...
      console.log("Error get Adverse Event by id : ", error)
    } finally {
      setLoading(false);
    }
  }
  ...
  const editAdverseEvent = async (data) => {
    setLoading(true);
    try {
      const res = await apiCall({
        method: "patch",
        url: `/admin/nhra/adverseEventList/updateAdverseEvent/${id}`,
        data: data
      })
      if (res.status === 200) {
      ...
      setError(error.message);
      console.log("Error update Adverse Event : ", error)
    } finally {
      setLoading(false);
    }
  }
...
export default EditAdverseEvent
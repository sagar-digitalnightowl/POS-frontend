...
const AllAlertModification = () => {
    ...
    const fetchAllAlertModification = async () => {
        setLoading(true);
        try {
            ...
            const res = await apiCall({
                method: "get",
                url: `/admin/nhra/alertModification/getAllAlertModification?${param.toString()}`,
            })
            if (res.status === 200) {
            ...
            console.log("Error get all alert and modification : ", error)
        } finally {
            setLoading(false);
        }
    }
    ...
    useEffect(() => {
        fetchAllAlertModification();
    }, [currentPage])


    const deleteAlertModification = async (id) => {
        setLoading(true);
        try {
            const res = await apiCall({
                method: "delete",
                url: `/admin/nhra/alertModification/deleteAlertModification/${id}`,
            })
            if (res.status === 200) {
                fetchAllAlertModification();
            } else {
            ...
            setError(error.message);
            console.log("Error delete alert and modification : ", error)
        } finally {
            setLoading(false);
        }
    }
            ...
            <div className="wrapper thetop" style={textStyle}>
                ...
                <div className=" content-wrapper ">
                    <section className="content-header">
                        <h1>Alerts & Modification </h1>
                    </section>
                    <section className="content">
                        <div className="box box-primary">
                            <div className="box-header">
                                <h3 className="box-title"> All Alerts & Modification </h3>
                                <div className="box-tools">
                                    <Link to="/alert-modification/create" className="btn btn-block btn-primary">
                                    ...
                                    <table className="table table-bordered table-striped ar_table" id="all_ar">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Change</th>
                                                <th>Modification</th>
                                                <th>Authorize Representative Name</th>
                                            ...
                                            {data.length > 0 ? (data.map((group, index) => (
                                                <tr key={index}>
                                                    ...
                                                    <td>
                                                        ...
                                                        &nbsp;
                                                        <Link to={`/alert-modification/edit/${group?._id}`} className="btn bg-purple btn-sm btn-flat" title="Edit" style={{ marginBottom: 3 }}>
                                                            <FontAwesomeIcon icon={faEdit} />
                                                        </Link>
                                                        &nbsp;
                                                        <button onClick={() => deleteAlertModification(group?._id)} className="btn btn-warning btn-sm btn-flat deletebtn" id="delete_product0" >
...
export default AllAlertModification
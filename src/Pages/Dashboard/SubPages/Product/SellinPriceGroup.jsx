...
const SellinPriceGroup = () => {
    ...
    const handleSave = () => {
        const newData = {"name": name,"description" : description };
        addSellingPriceGroup(newData);
        setName('');
        setDescription('');
        setShowModal(false);
    };
    ...
    const handleEdit = (id) => {
        setId(id);
        getSellingPriceGroup(id);
      }
  
      const handleEditSave = () => {
        const newData = { "name" : name, "description" : description };
        editSellingPriceGroup(newData);
        setShowEditModal(false);
        setName('');
        setDescription('');
      }

    const fetchSellingPriceGroup = async () => {
        try {
          setLoading(true);
          const response = await fetch(`${process.env.REACT_APP_BASE_URL}/admin/product/sellingPriceGroup/getsellingPriceGroup`, {
            method: 'GET',
            headers: {
          ...
          if (!response.ok) {
            throw new Error('Failed to fetch selling price group');
          }
      ...
      const getSellingPriceGroup = async (id) => {
        try {
          console.log("api");
          
          setLoading(true);
          const response = await fetch(`${process.env.REACT_APP_BASE_URL}/admin/product/sellingPriceGroup/getsellingPriceGroup/${id}`, {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
              // You may need to add authentication headers, like Bearer token, if required
  ...
  const addSellingPriceGroup = async (data) => {
    try {
      console.log(data);
      
      setLoading(true);
  
      const response = await fetch(`${process.env.REACT_APP_BASE_URL}/admin/product/sellingPriceGroup/addSellingPriceGroup`, {
        method: 'POST',
      ...
      const result = await response.json();
      toast.success("Selling price group added successfully");
  ...
  const editSellingPriceGroup = async (data) => {
    try {
      setLoading(true);
  
      const response = await fetch(`${process.env.REACT_APP_BASE_URL}/admin/product/sellingPriceGroup/updateSellingPriceGroup/${id}`, {
        method: 'PATCH',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify(data)
      });
      ...
      const result = await response.json();
      toast.success("Selling price group updated successfully");
      setShowEditModal(false);
    ...
    const deleteSellingPriceGroup = async (id) => {
        try {
          setLoading(true);
          const response = await fetch(`${process.env.REACT_APP_BASE_URL}/admin/product/sellingPriceGroup/deleteSellingPriceGroup/${id}`, {
            method: 'DELETE',
            headers: {
              'Content-Type': 'application/json',
              // You may need to add authentication headers, like Bearer token, if required
                    ...
                    <section className="content">
                        <div className="box box-primary">
                            <div className="box-header">
                                <h3 className="box-title">All Selling Price Group</h3>
                                ...
                                <br />
                                <small>
                                    Set multiple prices for products. Name different prices and then
                                    update prices from "Update Price" or List Products -&gt; Actions
                            ...
                            <div className="box-body">
                                <div className="table-responsive">
                                    <table className="table table-bordered table-striped" id="selling_price_group_table">
                                        <thead>
                                            <tr>
                                                <th>Name</th>
                                                <th>Description</th>
                                                    ...
                                                    <tr key={index}>
                                                        ...
                                                        <td style={textStyle}>
                                                            <DropdownButton id="dropdown-basic-button" title="Actions">
                                                                ...
                                                                <Dropdown.Item href="#/delete" onClick={()=>deleteSellingPriceGroup(group._id)}> <FontAwesomeIcon icon={faTrash} /> Delete</Dropdown.Item>
            ...
            {showModal && (
                <div className="modal show" style={{ display: 'block' }} tabIndex={-1} role="dialog">
                    <div className="modal-dialog" role="document">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h4 className="modal-title">Add Selling Product</h4>
            ...
            {showEditModal && (
                <div className="modal show" style={{ display: 'block' }} tabIndex={-1} role="dialog">
                    <div className="modal-dialog" role="document">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h4 className="modal-title">Edit Selling Product</h4>
                                <button type="button" className="close" onClick={handleModalClose}>
                                    <span>&times;</span>
                                </button>
                            </div>
...
export default SellinPriceGroup;

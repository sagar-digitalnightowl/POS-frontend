...
const EditDeliveryNote = () => {
  ...
  const fetchDeliveryNote = async () => {
    ...
    try {
      const res = await apiCall({
        method: "get",
        url: `/admin/sell/deliveryList/getDeliveryNote/${id}`,
      });

      if (res.status === 200) {
        setFormData({ ...res?.data?.result, saleOrder: res?.data?.result?.saleOrder?._id });
      } else {
        setError(res?.data?.message);
      }
    } catch (error) {
      setError(error.message);
      console.log("Error in fetch delivey note : ", error);
    } finally {
      setLoading(true);
    }
  }
  ...
  const editDeliveryNote = async (data) => {
    setLoading(true);
    try {
      const res = await apiCall({
        method: "patch",
        url: `/admin/sell/deliveryList/updateDeliveryNote/${id}`,
        data: data,
      });
      ...
      setError(error.message);
      console.log("Error in edit delivery note : ", error);
    } finally {
      setLoading(true);
    }
  };
                  ...
                  <div className="row">
                    ...
                    <div className="form-group col-md-4">
                      ...
                      <input
                        ...
                        defaultValue=""
                        required=""
                        value={formData.deliveryDate.substring(0, 10) || ""}
                        onChange={handleChange}
                      />
                    </div>

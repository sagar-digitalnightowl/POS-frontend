...
const StockTransfer = () => {
    ...
    const getAllStockTransfers = async () => {
        ...
        try {
            ...
            const res = await apiCall({ method: "get", url: `/admin/stockTransfer/stockList/getAllStockTransfer?${params.toString()}` });
            ...
            console.log("Error in fetch all stock transfer : ", error)
        } finally {
            setLoading(true);
        }
    }
    ...
    const deleteStockTransfer = async (id) => {
        setLoading(true);
        try {
            const res = await apiCall({ method: "delete", url: `/admin/stockTransfer/stockList/deleteStockTransfer/${id}` });
            ...
            setError(error.message);
            console.log("Error in delete stock transfer : ", error)
        } finally {
            setLoading(true);
        }
    }
                    ...
                    <section className="content no-print">
                        <div className="box box-primary">
                            <div className="box-header">
                                <h3 className="box-title">All Stock Transfers</h3>
                            ...
                            <div className="box-body">
                                <div className="table-responsive">
                                    <table
                                        className="table table-bordered table-striped ajax_view"
                                            ...
                                            {data.length > 0 ? (
                                                data.map((group, index) => (
                                                    <tr key={index}>
                                                        ...
                                                        <td>{group.locationFrom}</td>
                                                        <td>{group.locationTo}</td>
                                                        <td>{group.status}</td>
                                                        <td>{group.shippingCharges}</td>
                                                        <td>{group.totalStockTransferAmount}</td>
                                                        <td>{group.additionalNotes}</td>
                                                        <td>
                                                            <DropdownButton
                                                            ...
                                                            >
                                                                ...
                                                                <Dropdown.Item
                                                                onClick={() => navigate(`/stock-transfers/edit/${group?._id}`)}
                                                                >

...
const SalesReport = () => {
  ...
  const fetchAllSaleReports = async () => {
    setLoading(true);
    try {
      ...
      const res = await apiCall({
        method: "get",
        url: `/admin/report/saleReport/getAllSaleReports?${params.toString()}`,
      })
      ...
      console.log("Error in fetch sale reports : ", error)
    } finally {
      setLoading(false);
    }
  }
            ...
            <div className="box box-primary">
              <div className="box-body">
                ...
                <div className='table-responsive'>
                  <table className="table table-bordered table-striped ajax_view" id="sale_report_table">
                    <thead>
                      <tr>
                        <th>Report ID</th>
                        <th>Customer name</th>
                      ...
                      {data.length > 0 ?
                        data.map((group, index) => (
                          <tr key={index}>
                            ...
                            <td style={textStyle}>{group?.sale?.customer?.firstName} {group?.sale?.customer?.middleName} {group?.sale?.customer?.lastName}</td>
                            <td style={textStyle}>{group?.sale?.invoiceNo}</td>
                            <td style={textStyle}>{group?.sale?.saleDate}</td>
                            <td style={textStyle}>{group?.sale?.products?.reduce((acc, product) => acc + product.totalAmount, 0)}</td>
                            <td style={textStyle}>{group?.sale?.products?.reduce((acc, product) => acc + product.totalAmount, 0) - (
                              group?.sale?.discountType === "Percentage" ?
                                (group?.sale?.products?.reduce((acc, product) => acc + product.totalAmount, 0) *
                                  (group?.sale?.discountAmount / 100)).toFixed(2)
                                : group?.sale?.discountType === "Fixed" ?
                                  group?.sale?.discountAmount
                                  : 0
                            )}</td>
                            <td style={textStyle}>{
                              group?.sale?.discountType === "Percentage" ?
                            ...
                            <td style={textStyle}>{group?.sale?.orderTax}</td>
                            <td style={textStyle}>
                              {group?.sale?.products?.reduce((acc, product) => acc + product.totalAmount, 0) - (
                                group?.sale?.discountType === "Percentage" ?
                                  (group?.sale?.products?.reduce((acc, product) => acc + product.totalAmount, 0) *
                            ...
                            <td style={textStyle}>{group?.sale?.payments?.paymentMethod}</td>
                            <td style={textStyle}>{group?.sale?.payments?.amount}</td>
                          </tr>
                        ))
                        : <tr>

...
const EditExpenses = () => {
    ...
    const fetchExpenseById = async () => {
        ...
        try {
            const res = await apiCall({
                method: "get",
                url: `/admin/expense/addExpense/getExpense/${id}`
            });

            if (res.status === 200) {
                setFormData({ ...res?.data?.result, attachDocument: null })
            } else {
                setError(res?.data?.message);
            }
        } catch (error) {
            setError(error.message);
            console.log("Error get expense by id : ", error);
        } finally {
            setLoading(true);
        }
    }
    ...
    const editExpenses = async (data) => {
        ...
        try {
            const res = await apiCall({
                method: "patch",
                url: `/admin/expense/addExpense/updateExpense/${id}`,
                data: data
            });
